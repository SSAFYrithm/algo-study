...
public class SWEA5650_핀볼게임 {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int test_case = 1; test_case <= t; test_case++) {
			...
			board = new int[n + 2][n + 2]; //입력인덱스시작이 1~n이고, 벽에 부딪히는 로직을 위해 배열전체를 0으로 감싸준다.
...
//				System.out.println("작업전current 위치 : ("+current[0]+", "+current[1]+")"+", dir : "+dir+" ny,nx : ("+ny+", "+nx+")");
//				if(ny == 0 && nx == 10) System.exit(0);
//				if(ny == 0 && nx == 0) System.exit(0);;
				if(ny == 0 || nx == 0 || ny == n+1 || nx == n+1 || board[ny][nx] == 5) { // 벽이거나 5번블록일 때
...
//					System.out.println("4블록");
				...
				else if(board[ny][nx] == -1 || (ny == y && nx ==x) ) { // 종료 조건
